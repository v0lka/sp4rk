...
| `ToolJudger` | tools | Optionally implemented by a tool | Tool-specific safety heuristic: `Judge(ctx, input) JudgeOutcome{Allow, Reason, Severity}` — severity classifies the reason as `hard` (fired security control: blacklist, SSRF) or `soft` (scope question: path containment, evaluated on the symlink-resolved path). The severity reaches the host via `ConfirmationRequest.JudgeSeverity` (zero value `hard`), which hosts use to decide whether an escalation can be auto-resolved |
