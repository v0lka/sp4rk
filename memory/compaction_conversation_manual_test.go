...
func TestCompactConversationHistory_Empty(t *testing.T) {
	out, err := CompactConversationHistory(context.Background(), nil, "sliding_window", CompactionConfig{}, CompactionDeps{})
	if err != nil {
...
func TestCompactConversationHistory_UnknownStrategyFailsClosed(t *testing.T) {
	if _, err := CompactConversationHistory(context.Background(), convHistory(3), "bogus", CompactionConfig{}, CompactionDeps{}); err == nil {
		t.Fatal("expected error for unknown strategy")
	}
}

func TestCompactConversationHistory_SummarizationRequiresSummarizer(t *testing.T) {
	// A history that needs summarizing requires a non-nil Summarize.
	if _, err := CompactConversationHistory(context.Background(), convHistory(10), "summarization", CompactionConfig{}, CompactionDeps{}); err == nil {
		t.Fatal("expected error when Summarize is nil for summarization")
	}
	if _, err := CompactConversationHistory(context.Background(), convHistory(10), "hierarchical", CompactionConfig{}, CompactionDeps{}); err == nil {
		t.Fatal("expected error when Summarize is nil for hierarchical")
	}
}

func TestCompactConversationHistory_ShortHistoryNeedsNoSummarizer(t *testing.T) {
	// A history the LLM-backed strategies return verbatim needs no LLM: a nil
	// Summarize is NOT an error for it — nothing to summarize. A 2-message
	// history is under keepLast=5 (summarization) AND has empty summary zones
	// (hierarchical: distant=middle=0 at n=2), so both return it verbatim.
	short := convHistory(1) // 2 messages
	if _, err := CompactConversationHistory(context.Background(), short, "summarization", CompactionConfig{}, CompactionDeps{}); err != nil {
		t.Fatalf("short history must not require Summarize: %v", err)
	}
	if _, err := CompactConversationHistory(context.Background(), short, "hierarchical", CompactionConfig{}, CompactionDeps{}); err != nil {
		t.Fatalf("short history must not require Summarize (hierarchical): %v", err)
	}
}
...
func TestCompactConversationHistory_NeverRemovesLastMessage(t *testing.T) {
	...
	for _, strategy := range []string{"sliding_window", "summarization", "hierarchical"} {
		...
		out, err := CompactConversationHistory(context.Background(), msgs, strategy, cfg, deps)
		if err != nil {
...
func TestCompactConversationHistory_ShortHistoryNoop(t *testing.T) {
	...
	for _, strategy := range []string{"sliding_window", "summarization"} {
		...
		out, err := CompactConversationHistory(context.Background(), msgs, strategy, CompactionConfig{}, deps)
		if err != nil {
			t.Fatalf("strategy %s: unexpected error: %v", strategy, err)
		}
		if len(out) != len(msgs) {
...
func TestCompactConversationHistory_SummarizationPropagatesError(t *testing.T) {
	...
	if _, err := CompactConversationHistory(context.Background(), msgs, "summarization", cfg, deps); err == nil {
...
func TestCompactConversationHistory_Hierarchical(t *testing.T) {
	...
	out, err := CompactConversationHistory(context.Background(), msgs, "hierarchical", cfg, deps)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestCompactConversationHistory_HierarchicalShortHistoryNoop(t *testing.T) {
	...
	out, err := CompactConversationHistory(context.Background(), msgs, "hierarchical", CompactionConfig{}, deps)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
