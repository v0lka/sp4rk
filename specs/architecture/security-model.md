...
The criteria are evaluated in fixed priority (C1 > … > C10; all fired criteria are recorded in the digest, and the highest-severity fired criterion — ties broken by that priority — sets the outcome). Scores/grades are never thresholds — only structural facts (effect kinds, targets, destructive KB class, exfiltration pairs) fire criteria:
