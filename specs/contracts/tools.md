...
| `ToolJudger` | tools | Optionally implemented by a tool | Tool-specific safety heuristic: `Judge(ctx, input) JudgeOutcome{Allow, Reason, Severity, ReasonCode}` — severity classifies the reason as `hard` (fired security control: blocklist pattern, a flowsh criterion, SSRF) or `soft` (scope question: path containment / credential access, evaluated on the symlink-resolved path). The severity reaches the host via `ConfirmationRequest.JudgeSeverity` (zero value `hard`), which hosts use to decide whether an escalation can be auto-resolved |
| `JudgeReasonCode` | tools | Set by judges, read by hosts | Stable, machine-checkable classification of an escalation's reason, delivered as `ConfirmationRequest.JudgeReasonCode`. Published codes: `command_blacklist`, `outside_session_roots`, `ssrf_private_address`, `ssrf_protection_degraded`, `unassessable_url`, `unassessable_path`, `git_internal_path`, `symlink_escape`, `symlink_suspicious`, and the flowsh criteria `command_exfil_flow`, `command_privilege_escalation`, `command_system_write`, `command_destructive_outside_roots`, `command_download_cradle`, `command_unbounded_analysis`, `credential_access`, `command_analysis_unavailable` (`unresolvable_path_token` is retained as a published code for contract stability but no built-in judge fires it). The prose `Reason` may be reworded freely; a published code is never renamed or reused — new classifications add new codes. The empty value means unclassified: hosts decide unclassified escalations by their own fail-closed policy, never by matching prose |
