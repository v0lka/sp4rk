...
- **Failure mode**: `DetectToolCallSyntaxInContent` matches the model "printing" a tool invocation as text instead of emitting a `tool_use` block — either a fenced code block whose language tag looks like a tool name (`` ```bash_exec ``), or a lone JSON tool-call envelope printed as the whole response (the finish args `{"answer": "..."}`, a `{"name": ..., "arguments": {...}}` / `{"tool": ..., "args": {...}}` pair, or an OpenAI-style `{"function": {"name": ..., "arguments": ...}}`), optionally wrapped in one ```` ```json ```` fence. A dedicated nudge is injected a few times; after that the executor aborts with `Finished: false` (never a silent success). This is what prevents a leaked `{"answer": "..."}` from being surfaced verbatim as the final answer.
