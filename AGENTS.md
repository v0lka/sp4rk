...
- **Model registry resolution:** `Resolve` is a 6-tier case-insensitive lookup (overrides → built-in → fuzzy vendor-prefix/separator-insensitive → cache → HuggingFace/registered sources → fallback with optimistic `Attachment`). `ResolveBuiltInModel` is catalog-only (no network); `SetCachedMetadata` stores late-learned metadata at the cache tier. A **partial** override (one that leaves some scalar fields unset, e.g. a protocol-only `{Protocol: ChatCompletions}`) inherits its unset `ContextWindow`/`OutputLimit`/`TokenizerType`/`Capabilities` from the lower non-network tiers — a zero `ModelCapabilities` counts as "unset" — so pinning the protocol does not collapse the context window or disable capabilities. A fully-specified override is returned verbatim. The fuzzy tier is now O(1) via normalized-ID indexes built at construction.
