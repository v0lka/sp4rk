...
// TestNewEmbedder_NegativeDeviceID verifies the loud rejection of a negative
// DeviceID for every execution provider: unlike an out-of-range positive
// ordinal (a runtime CUDA rejection), a negative value is a configuration
// error that must fail during validation, before any ONNX lifecycle is
// touched.
func TestNewEmbedder_NegativeDeviceID(t *testing.T) {
	destroys := onnxEnvDestroys.Load()
	for _, provider := range []string{ExecutionProviderCPU, ExecutionProviderCUDA, ExecutionProviderAuto} {
		_, err := NewEmbedder(EmbedderConfig{
			ModelPath:         "m.onnx",
		...
		if err == nil {
			t.Errorf("NewEmbedder(provider=%q, DeviceID=-1) error = nil, want non-nil", provider)
			continue
		}
		if !strings.Contains(err.Error(), "-1") {
			t.Errorf("NewEmbedder(provider=%q, DeviceID=-1) error = %v, want it to name the value", provider, err)
		}
		if !strings.Contains(err.Error(), "DeviceID") {
			t.Errorf("NewEmbedder(provider=%q, DeviceID=-1) error = %v, want it to name the field", provider, err)
		}
	}
	if got := onnxEnvDestroys.Load() - destroys; got != 0 {
		t.Errorf("destroyONNXRuntime called %d times during rejected configs, want 0", got)
...
// provider (validation), a negative DeviceID (validation, every provider),
// a tokenizer load failure (after env init and options build — the deepest
// cleanup(sessOpts) path), and an explicit "cuda" that fails (the loudest
// historical failure).
func TestNewEmbedder_FailurePaths_NeverDestroyEnv(t *testing.T) {
	...
	// Negative DeviceID (validation): rejected for every provider before
	// ONNX is touched, unlike an out-of-range positive ordinal, which stays a
	// runtime CUDA rejection.
	for _, provider := range []string{ExecutionProviderCPU, ExecutionProviderCUDA, ExecutionProviderAuto} {
		if _, err := NewEmbedder(EmbedderConfig{
			ModelPath: "m.onnx", TokenizerPath: "t.json", LibraryPath: "lib.so",
			ExecutionProvider: provider,
			...
			t.Errorf("NewEmbedder(provider=%q, DeviceID=-1) expected error, got nil", provider)
		}
	}
