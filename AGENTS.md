...
- **Tool registry pattern:** built-in tools live in `tools/builtins/`; MCP-backed tools are added at runtime via `tools/mcp/gateway.go`. To add a built-in tool, implement the `tools.Tool` interface and register it on the `ToolRegistry`. **Capability-group contract (see `tools/group.go`)**: every new tool MUST declare its capability group — set `ToolGroup` on the embedded `BaseTool` (which supplies `Group()`), one of `execute`, `local_read`, `local_write`, `remote_read`, `remote_write`, `local_mcp`, `remote_mcp`, or the reserved `system` (orchestration/state tools with no direct filesystem/network effect — host-trusted builtins that bypass policy gates; never grant it to a tool touching external input without security review). The group is the only key hosts may policy-gate or budget subagent toolsets by — never tool names. An undeclared (zero) group matches no group allow-list: the tool becomes invisible to group-scoped consumers (fail-closed). MCP tools derive their group from the transport (`stdio` → `local_mcp`, `http` → `remote_mcp`) unless the operator pins a per-server `ToolGroupOverride` (`ServerEntry`/`ServerConfig`; empty/undeclared values fall back to the transport default, and the reserved `system` override is ignored — an external server's tools are never host-trusted).
...
- **Model registry resolution:** `Resolve` is a 7-tier case-insensitive lookup (overrides → observed runtime entries written by `SetRuntimeMetadata`, which supersede the built-in spec for how a model is actually served → built-in → fuzzy vendor-prefix/separator-insensitive, which also consults the runtime index → cache → HuggingFace/registered sources → fallback with optimistic `Attachment`). `ResolveLocal` is the strictly network-free variant (local tiers only) for synchronous UI paths; `ResolveBuiltInModel` is catalog-only (no network); `SetCachedMetadata` stores late-learned metadata at the cache tier; `RuntimeMetadata` reads a runtime entry back as stored (un-enriched). A failed HuggingFace probe is negatively cached for 10 minutes (`context.Canceled` from the caller excepted; `Invalidate` clears both entries). A **partial** override (one that leaves some scalar fields unset, e.g. a protocol-only `{Protocol: ChatCompletions}`) inherits its unset `ContextWindow`/`OutputLimit`/`TokenizerType`/`Capabilities` from the lower non-network tiers — `Capabilities` is a pointer (`*ModelCapabilities`): nil counts as "unset" (inherited), non-nil is authoritative even when all-false — so pinning the protocol does not collapse the context window or disable capabilities. A fully-specified override is returned verbatim. Resolvers always return non-nil, defensively copied `Capabilities` (registry state, including the process-wide built-in catalog, is never aliased); records handed to the registry (overrides map, `SetCachedMetadata`, `SetRuntimeMetadata`) are copied on write. The fuzzy tier is O(1) via normalized-ID indexes built at construction (the runtime index is rebuilt on every runtime write).
