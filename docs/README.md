...
The SDK gives you a production-grade foundation for agents that reason, act, and self-correct: a ReAct execution loop with cooperative pause, live user steering, resumable interjections, and optional verify-on-edit; a DAG-based planner; a reflector for failure analysis; purpose-aware multi-provider LLM routing; a thread-safe capability-grouped tool registry; Model Context Protocol (MCP) integration; context-window management with automatic compaction; human-in-the-loop hooks; structured event streaming; skill discovery; ONNX-based local embeddings; and prompt-injection defenses.
...
- **Multi-provider LLM routing** — Anthropic, OpenAI Chat Completions, OpenAI Responses, and Google generateContent behind a single `Router`. Purpose-aware sampling applies vendor presets to executor calls and deterministic profiles to routing, compaction, and summarization; provider adapters omit unsupported controls.
- **Custom & built-in tools** — implement the `tools.Tool` interface or use the bundled file, shell, search, web, and vector-search tools. Every tool declares a capability group used for fail-closed toolset selection and host policy; a thread-safe `ToolRegistry` holds them all.
...
- **Plan & Execute orchestration with DAG** — a `Planner` generates a directed-acyclic-graph of steps with dependencies and parallelism hints; a `Conductor` executes each step as its own ReAct loop, supports cooperative pause/resume, and can deliver one-shot resume interjections.
...
- **Human-in-the-loop and mechanical verification** — a `HITLHandler` intercepts tool calls for confirmation or modification and decides what happens when the step budget is reached. Optional verify-on-edit runs a trusted `EditVerifyRunner` once after each response group containing successful file edits.
...
- **Tool execution-context intelligence** — advisory `ToolJudge` evaluates mutating calls with session-aware caching; `JudgeStrict` performs a fresh, fail-safe assessment for soft confirmation gates. Severity prevents strict automation from overriding hard findings such as blacklist, SSRF, or unassessable-input violations; file-coherence and symlink checks protect session scope. See [Tool Safety & Execution Context](tool-safety.md).
...
| [llm-providers.md](llm-providers.md) | Multi-provider routing, purpose-aware sampling, ModelRegistry/runtime metadata, and provider configuration |
| [tools.md](tools.md) | The `Tool` interface, capability groups, `ToolRegistry`, built-in tools, and custom tools |
| [mcp-integration.md](mcp-integration.md) | Model Context Protocol servers, the gateway, and external tool discovery |
| [agent-executor.md](agent-executor.md) | The `Executor`, ReAct lifecycle, cooperative pause/live steering, verify-on-edit, and circuit breakers |
...
| [orchestration.md](orchestration.md) | The `Conductor`, blackboard, pause/resume checkpoints, interjections, and single-task execution |
...
| [agents.md](agents.md) | Subagent Profiles — `AGENT.md` discovery, capability-group tool budgets, and per-agent models |
| [security.md](security.md) | Prompt injection defense and untrusted-content wrapping |
| [tool-safety.md](tool-safety.md) | Advisory/strict `ToolJudge`, severity, session-aware path safety, file coherence, environment info, and symlink detection |
...
| `…/agent` | ReAct `Executor` loop, cooperative pause/live-message hooks, verify-on-edit, resumable interjections, `Events`, `HITLHandler`, `FinishTool`, `RunSubAgent`, tool-result cache |
...
| `…/llm` | `Router`, `Provider`, `ProviderEntry`, `CallPurpose`/`SamplingDefaults`, `ModelRegistry`, token counting, Anthropic/OpenAI/Google protocol adapters |
| `…/tools` | `Tool` interface, `BaseTool`, capability groups, thread-safe `ToolRegistry`, `ToolPolicy`, `ToolDescriptor`, advisory/strict `ToolJudge`, file coherence, `EnvInfo`, symlink detection |
