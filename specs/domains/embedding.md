...
| `auto` | Prefer CUDA, degrade gracefully. `NewEmbedder` attempts CUDA (on the dedicated runner thread); when the attempt fails — CPU-only build, missing driver, an out-of-range (non-negative) device id, or a session that cannot be created on the GPU — it logs a WARN and continues on the CPU provider | Falls back to CPU; only an environment-init or CPU-path failure returns an error |
...
- **Session stage (fixed mode: the eager session; bucket mode: the first lazy session).** In fixed mode, when the CUDA options were accepted but the eager query session itself could not be built on the GPU, `NewEmbedder` retries once on the CPU provider (options rebuilt, session re-created) before giving up; the final error chains both the original CUDA error and the CPU retry error. In bucket mode the eager session is not created, so the same retry covers the **first** lazily created bucket (or batch) session: when it cannot be built on the GPU the embedder degrades to the CPU provider (options rebuilt, ONNX thread stopped) and retries. Degradation only happens while no CUDA-backed session is live — a later session failure with an already-built CUDA session surfaces as a loud error (`creating persistent ONNX session for length bucket %d`) rather than stranding that live session's dedicated thread.
...
Two diagnostic helpers answer "what GPUs exist" and "is this process actually on the GPU" without depending on ONNX Runtime self-reporting. Both shell out to `nvidia-smi` and accept a caller-supplied `context.Context` as their first parameter. Both derive their working context via `context.WithTimeout(ctx, gpuProbeTimeout)`, so the effective budget is **min(caller deadline, 2s)** — an earlier caller deadline always wins, while the shared 2s cap remains the upper bound for callers without one. A wedged driver stalls neither. Both also suppress the Windows console window a GUI-subsystem host would allocate for the probe child process (`sysproc.HideConsole`, a no-op on other platforms). Result semantics mirror each other: a missing `nvidia-smi` yields a no-error empty result (absence of the tool is not an error, and the absent-tool fast path returns without consulting `ctx`), while a failed invocation — including a cancelled or expired context once the probe is actually running — returns an error.

- **`GPUInUse(ctx)`** runs `nvidia-smi --query-compute-apps=pid --format=csv,noheader` and reports whether the driver registers *this* process (by PID) as a CUDA compute application. Callers treat an error as "unverified", never as "not on GPU"; a cancelled/expired `ctx` is exactly such an error. A CUDA context materializes lazily (first inference on the CUDA provider) and lives until the ONNX Runtime environment is destroyed, so the correct moment to probe is after at least one real inference. In PID-namespaced environments (containers, WSL) NVML may report host-namespace PIDs that never match `os.Getpid()` — a known limitation.
- **`ListGPUDevices(ctx)`** runs `nvidia-smi --query-gpu=index,name --format=csv,noheader` and returns one `GPUDevice{Index, Name}` per parseable line; `Index` is the driver-assigned ordinal that matches `EmbedderConfig.DeviceID` and `CUDA_VISIBLE_DEVICES`. Empty or prose-only output yields an empty, non-nil slice. As with `GPUInUse`, a cancelled/expired `ctx` surfaces as an error rather than a silently empty device list.
...
**Normalization-panic robustness.** sugarme/tokenizer v0.3.0's `NormalizedString` alignment bookkeeping assumes BERT normalization preserves the original byte layout; any normalizer step that changes it desynchronizes the offsets and makes the library panic (slice-bounds in `TransformRange`, nil-pointer in `Slice` via `AddedVocabulary.splitWithIndices`). On the jina-v2-small config two steps do this — `cleanText`, which removes NUL (U+0000), U+FFFD and control runes, and `lowercase`, which can change a rune's byte length (e.g. U+023A → U+2C65). Both trigger on **valid** UTF-8, so this is not an invalid-UTF-8 problem; malformed bytes are only one route into the `cleanText` class, since ranging over a string yields U+FFFD for every undecodable byte. `EncodeWithLength` defends in two layers: (A) `sanitizeForTokenizer` rewrites every `cleanText`-stripped rune — and every undecodable byte — to a space, which `cleanText` never removes; (B) if a panic still escapes (the lowercase-length class in particular), the encode is retried once on an ASCII-folded copy (`asciiFold`) so the document is indexed with a coarsened embedding instead of being dropped, and any residual panic is converted into an ordinary error.
...
- `ChunkFile` returns `nil` for binary files (a null byte in the first 512 bytes) and for empty input; chunks always carry location metadata (file path + 1-based line range).
- The persistent vector index/store is host-side; the SDK provides only the embedder, tokenizer, chunker, and the `semantic_search` tool that delegates to a host-provided search function.
- `semantic_search` returns empty results until the embedder/index is ready (the host-supplied wait function gates it).
...
- The `auto` CUDA→CPU fallback at the `buildSessionOptions` stage applies in every mode (fixed and bucket). The additional session-stage retry-on-CPU covers the eager session in fixed mode and the **first** lazily created bucket/batch session in bucket mode (only while no CUDA-backed session is live); a later session failure with an already-built CUDA session is a loud error, never a silent CPU retry.
...
- `GPUInUse(ctx)`/`ListGPUDevices(ctx)` take a caller-supplied context whose deadline composes with the shared 2s probe budget — min(caller deadline, 2s) wins. They treat a missing `nvidia-smi` as "no signal" (empty result, nil error, `ctx` not consulted) and a failed invocation — including a cancelled/expired context — as an error; both complete within that composed budget.
...
- **GPU acceleration**: set `ExecutionProvider` to `auto` (safe default when hardware varies) or `cuda` (loud failure when CUDA is expected), pair it with a CUDA-enabled ONNX Runtime build, and pick the device with `DeviceID` (enumerate candidates via `ListGPUDevices(ctx)`, whose `GPUDevice.Index` matches). Verify a running setup externally with `GPUInUse(ctx)` after the first inference — pass the caller's own context to bound the probe below the internal 2s cap where needed.
